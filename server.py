...
def build_job_from_payload(payload: dict) -> tuple[int, dict]:
    ...
    if not entries:
        ...
        if course:
            job = JOB_STORE.find_latest_by_course(client_id, course["course_key"])
            if job:
                return 200, {
                    "ok": True,
                    "mode": "query",
            ...
            return 404, {
                "ok": False,
                "mode": "query",
                "error": "没有找到这门课程的历史记录。第一次运行第三门课程时，请输入 token-课程链接。",
                "success_count": 0,
                "fail_count": 1,
                "logs": [
                    "[QUERY] 没有找到这门课程的历史记录。",
                    "[TIP] 第一次运行第三门课程时，请输入 token-https://higher.smartedu.cn/course/lmc/课程ID。",
                ],
            }
    ...
    if len(entries) == 1 and entries[0].target == TARGET_SMARTEDU_LMC:
        ...
        active_job = JOB_STORE.find_active_by_course(client_id, course_key) if course_key else None
        if active_job:
            return 200, {
                "ok": True,
                "mode": "attached",
